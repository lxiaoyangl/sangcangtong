  ...
  <div style="height:100%;">
    ...
    <el-form :inline="true" :model="formInline" class="search-box">
      <el-form-item label="资源目录名称">
        <el-input clearable placeholder="请输入资源目录名称" :maxlength="20" v-model.trim="formInline.name"></el-input>
      </el-form-item>
      <el-form-item label="状态">
        <el-select clearable v-model="formInline.catalogState" placeholder="请选择" >
    ...
    <div class="content-box mt10">
      ...
      >
        ...
        <el-table-column sortable prop="name" label="目录名称" min-width="200" show-overflow-tooltip>
          <template slot-scope="scope">
            <i v-if="scope.row.icon" class="scope.row.icon"></i>
            {{scope.row.name}}
          </template>
        </el-table-column>
        <el-table-column sortable prop="orderNumber" align="center" label="显示排序" min-width="110"></el-table-column>
        <el-table-column sortable prop="catalogState" align="center" label="状态" min-width="110">
          <template slot-scope="scope">
            <el-tag size="mini" :type="scope.row.catalogState===1?'':'danger'" effect="dark" v-if="states[scope.row.catalogState]">
              {{states[scope.row.catalogState].label}}
            </el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column sortable prop="menuState" align="center" label="可见" min-width="90">
          <template slot-scope="scope">
    ...
    <div v-if="editFormVisible" @click="showIcons=false">
      <el-dialog :close-on-click-modal="false"  :title="title" :visible.sync="editFormVisible" @click="editFormVisible=false" width="700px">
        <el-form label-width="80px" ref="editForm" :model="editForm" :rules="rules">
          <el-form-item label="上级目录" prop="parentName">
            <el-input :disabled="editForm.type==='view'" v-model="editForm.parentName" @focus="selectTree" :maxlength="20"
          ...
          <el-form-item label="目录名称" prop="name">
            <el-input :disabled="editForm.type==='view'" v-model.trim="editForm.name" auto-complete="off" :maxlength="50"
                      placeholder="请输入名称"></el-input>
          </el-form-item>
          <el-form-item label="排序" prop="orderNumber">
            <el-input :disabled="editForm.type==='view'" v-model.trim="editForm.orderNumber" type="number" :maxlength="50"
         ...
         <!-- <div v-if="editForm.menuType !== 4">
           ...
           <el-form-item label="资源状态" prop="catalogState">
             <el-radio :disabled="editForm.type==='view'" v-model="editForm.catalogState" :label="0">隐藏</el-radio>
             <el-radio :disabled="editForm.type==='view'" v-model="editForm.catalogState" :label="1">显示</el-radio>
           </el-form-item>
         <!-- </div> -->
          <el-form-item label="描述" prop="description">
            <el-input :disabled="editForm.type==='view'" type="textarea" :rows="2" v-model="editForm.description" :maxlength="200"
...
export default {
  data () {
    return {
      ...
      states: [{ label: '隐藏', value: 0 }, { label: '可见', value: 1 }],
      multipleSelection: [],
      showIcons: false,
      showTrees: false,
      treeDatas: null,
  ...
  methods: {
    getList () {
      ...
      setMaterialsList(sendData).then((res) => {
    ...
    updateState (data) {
      ...
      setMaterialsEdit(sendData).then((res) => {
        this.$message({
          message: res,
          type: 'success'
        })
    ...
    addList (editData) {
      this.$refs[editData].validate((valid) => {
        if (valid) {
          ...
          setMaterialsAdd(sendData).then((res) => {
            this.$message({
              message: res,
              type: 'success'
            })
    ...
    deleteItem (data) {
      sureDelete(() => {
        ...
        setMaterialsDel(sendData).then((res) => {
          this.$message({
            message: res,
            type: 'success'
          })
    ...
    batchDelete () {
      ...
      sureDelete(() => {
        setMaterialsDels(sendData).then((res) => {
          this.$message({
            message: res,
            type: 'success'
          })
