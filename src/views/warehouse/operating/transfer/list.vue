        ...
        <BasisPage>
            <el-form slot="form" ref="form" :model="form" :inline="true" label-width="100px">
                ...
                <el-form-item label="过入方">
                    <el-select clearable v-model="form.inp3" placeholder="请选择过入方" size="mini">
                        <el-option
...
export default {
    ...
    mounted(){
        ...
        sessionStorage.setItem('warehouse-transfer-edit','false');
        // 获取仓库数据
    ...
    methods:{
        ...
        examine(){
            ...
            //         arr.push(obj);
            //     })
            //     this.$axios.post('/applicationTransfer/baseBatchEdit',{
            //         list:arr
            //     }).then(res=>{
        ...
        get_data(){
            ...
            let url = '/applicationTransfer/baseList?_pageList&ascColumn=documentState,createTime';
            if(this.form.inp1 !== ''){
            ...
            if(this.form.inp3 !== null){
                senddata.receiverId = this.form.inp3;
            }
            if(this.form.inp4 !== null){
                // senddata.createTime = this.$fn.timeChange(this.form.inp4); // 计划装车时间
                url = url+'&createTime_begin='+this.$fn.timeChange(this.form.inp4[0]).substr(0,10)+'&createTime_end='+this.$fn.timeChange(this.form.inp4[1]).substr(0,10)
            }
            this.loading = true;
            this.$axios.post(url,{
                pageNo:1,
        ...
        del_data(){
            ...
            this.$confirm('即将删除订单, 是否继续?', '提示', {
                ...
                this.$axios.post('/applicationTransfer/delApplicationTransfer',{
                    ids:arr.join(',')
                }).then(res=>{
        ...
        change(){
            ...
            sessionStorage.setItem('warehouse-transfer-edit','true');
            sessionStorage.setItem('warehouse-transfer-aplicationid',this.table_selection_arr[0].id);
            this.$router.push({name:'transfer_add'});
