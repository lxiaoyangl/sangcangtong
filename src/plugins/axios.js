...
Vue.use(ElementUI);
import ipConfig from "Public/config.js";
...
let config = {
  ...
  baseURL: ipConfig.serverUrl,
};
const _axios = axios.create(config);

_axios.interceptors.request.use(
  function (config) {
    //判断是否为登录接口，未登录的话就在header里面加token
  ...
  return axios(config).then((res) => {
    ...
    if (sussArr.indexOf(res.data.status) > -1) {
      ...
      if (res.data.status !== 202 && res.data.status !== 201) {
        clearpop(res.data.data || res.data.msg, 'error')
      }
...
export function postApi(url, params) {
  return _commonApi('post', url, params)
}

export function getApi(url, params) {
  return _commonApi('get', url, params)
}
